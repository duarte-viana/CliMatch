...
# From Snell Taylor et al. 2020 (https://doi.org/10.1111/geb.13064)

setwd("~/Documents/Papers/iDiv/Paper_BBS_R2-traits")
load("BBS_BIOCLIM.Rdata")
bbs.wide <- spread(bbs.clim, key=species_id, value=abundance)
bbs.wide <- bbs.wide[,24:ncol(bbs.wide)]
bbs.wide[is.na(bbs.wide)] <- 0
comp <- read.csv("geb13064-sup-0006-tables1.csv")
comp$Focal.AOU <- as.character(comp$Focal.AOU)
comp$Competitor.AOU <- as.character(comp$Competitor.AOU)

ci <- data.frame(aou=unique(comp$Focal.AOU),c.all=NA,c.widespread=NA,c.max=NA)
for(i in unique(comp$Focal.AOU)){
  if(i %in% names(bbs.wide)){
    abund.focal <- bbs.wide[,i]
    wc <- comp$Competitor.AOU[comp$Focal.AOU==i]
    wc <- wc[wc %in% names(bbs.wide)]
    if(wc %in% names(bbs.wide)  && length(wc)>0){
      abund.sum <- apply(as.data.frame(bbs.wide[,wc]),1,sum)
      c.all <- mean(abund.focal/(abund.sum+abund.focal),na.rm=T)
    } 
    wc.widespread <- comp$Competitor.AOU[comp$Focal.AOU==i & comp$Most.Widespread.Competitor==1]
    if(wc.widespread %in% names(bbs.wide)  && length(wc.widespread)==1){
      abund.widespread <- bbs.wide[,wc.widespread]
      c.widespread <- mean(abund.focal/(abund.widespread+abund.focal),na.rm=T)
    } 
    wc.max <- comp$Competitor.AOU[comp$Focal.AOU==i & comp$Most.Predictive.Competitor==1]
    if(wc.max %in% names(bbs.wide) && length(wc.max)==1){
      abund.max <- bbs.wide[,wc.max]
      c.max <- mean(abund.focal/(abund.max+abund.focal),na.rm=T)
    } 
    ci[ci$aou==i,2:4] <- c(c.all,c.widespread,c.max)
  }
}

save(ci,file="competition_index.Rdata")
